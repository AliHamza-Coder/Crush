...
fn render_header(f: &mut Frame, area: Rect, app: &App) {
    ...
    let ff_pill = if app.ffmpeg.is_some() {
        "FFmpeg ●"
    } else {
        "FFmpeg ○"
    };
    let onnx_pill = if app.ai.is_model_available() {
        "ONNX ●"
    } else {
        "ONNX ○"
    };
...
fn render_analysis(f: &mut Frame, area: Rect, app: &App) {
    ...
    if let Some(ref stats) = app.stats {
        ...
        if stats.total_size > 0 {
            ...
            let colors: Vec<Color> = std::iter::repeat_n(Color::Green, si)
                .chain(std::iter::repeat_n(Color::Cyan, sv))
                .chain(std::iter::repeat_n(Color::Yellow, sa))
                .collect();
            let spans: Vec<Span> = bar
                .chars()
                .zip(colors)
                .map(|(c, cl)| Span::styled(c.to_string(), Style::default().fg(cl)))
                .collect();
        ...
        // ── format chips ──
        let format_order = [
            ".jpg", ".jpeg", ".png", ".webp", ".avif", ".gif", ".svg", ".mp4", ".mov", ".webm",
            ".mkv", ".avi", ".mp3", ".wav", ".flac", ".ogg", ".m4a", ".aac", ".opus", ".alac",
        ];
        ...
        if stats.total == 0 {
            ...
            lines.push(Line::from(vec![Span::styled(
                "  Click \"Change dir…\" to browse",
                Style::default().fg(Color::DarkGray),
            )]));
        }

        let list = List::new(lines.into_iter().map(ListItem::new).collect::<Vec<_>>()).block(block);
...
fn render_menu(f: &mut Frame, area: Rect, app: &App) {
    ...
    f.render_stateful_widget(
        list,
        area,
        &mut ListState::default().with_selected(Some(app.menu_idx)),
    );
}
...
fn render_files_view(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    let block = Block::default()
        .borders(Borders::ALL)
        .border_style(Style::default().fg(if app.select_mode {
            Color::Yellow
        } else {
            Color::Cyan
        }))
    ...
    if files.is_empty() {
        f.render_widget(
            Paragraph::new("No files in this directory").block(block),
            area,
        );
        return;
    }
    ...
    let mut items = vec![ListItem::new(header_line), ListItem::new(Line::from(""))];
    ...
    for (i, file) in files.iter().enumerate() {
        ...
        items.push(ListItem::new(Line::from(vec![
            ...
            Span::styled(
                format!("{:<6} ", file.ext.trim_start_matches('.').to_uppercase()),
                style,
            ),
    ...
    f.render_stateful_widget(
        list,
        area,
        &mut ListState::default().with_selected(Some(app.selected_file)),
    );
}
...
fn render_quality_view(f: &mut Frame, area: Rect, app: &App) {
    ...
    let mut list_items: Vec<ListItem> = qualities
        .iter()
        .enumerate()
        .map(|(i, &(q, label, desc, color))| {
            ...
            ListItem::new(Line::from(vec![
                Span::styled(
                    format!("  {} ", q_str),
                    Style::default().fg(color).add_modifier(if sel {
                        Modifier::BOLD
                    } else {
                        Modifier::empty()
                    }),
    ...
    f.render_stateful_widget(
        list,
        area,
        &mut ListState::default().with_selected(Some(app.quality_idx)),
    );
}
...
fn render_format_view(f: &mut Frame, area: Rect, app: &App) {
    ...
    f.render_stateful_widget(
        list,
        area,
        &mut ListState::default().with_selected(Some(app.format_idx)),
    );
}
...
fn render_queue_view(f: &mut Frame, area: Rect, app: &App) {
    ...
    let pct_val = if state.total > 0 {
        (done as f64 / state.total as f64 * 100.0) as u16
    } else {
        0
    };
    ...
    let rows: Vec<ListItem> = state
        ...
        .map(|task| {
            ...
            ListItem::new(Line::from(vec![
                ...
                Span::styled(
                    format!("{:<16} ", trunc(&task.file_name, 16)),
                    Style::default().fg(Color::White),
                ),
                Span::styled(bar, Style::default().fg(engine_color)),
                Span::styled(
                    format!(" {:>5.0}% ", task.progress),
                    Style::default().fg(Color::DarkGray),
                ),
                Span::styled(
                    format!("{:<4} ", engine_name),
...
fn render_quality_submenu(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    f.render_stateful_widget(
        list,
        card,
        &mut ListState::default().with_selected(Some(app.submenu_idx)),
    );
}
...
fn render_help(f: &mut Frame, area: Rect, _app: &App) {
    ...
    let sections: Vec<(&str, Vec<(&str, &str)>, Color)> = vec![
        ...
        (
            "QUEUE",
            vec![("c", "Cancel processing"), ("x", "Clear queue")],
            Color::Yellow,
        ),
        (
            "CLI",
    ...
    let hint = match app.active_panel {
        ...
        Panel::Files => {
            if app.select_mode {
                ...
                "↑↓ navigate  Click select  Tab back"
            }
        }
